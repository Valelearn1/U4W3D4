...
import valentinaferro.u4w3d4.entities.BlogPost;
...
public class BlogPostService {
	...
	public BlogPostResponseDTO create(BlogPostRequestDTO dto) {
		...
		User autore = userService.findById(dto.autoreId());
	...
	public List<BlogPostResponseDTO> findByAutoreId(Long autoreId) {
		userService.findById(autoreId); // verifica che l'autore esista, altrimenti 404
	...
	public BlogPostResponseDTO update(Long id, BlogPostRequestDTO dto, User currentUser) {
		BlogPost esistente = findEntityById(id);

		// AUTORIZZAZIONE: SOLO L'AUTORE del post può modificarlo (nemmeno un ADMIN).
		// currentUser è l'utente autenticato messo nel SecurityContext da AuthFilter e passato
		// dal controller con @AuthenticationPrincipal. Se non è l'autore -> AccessDeniedException -> 403.
		if (!esistente.getAutore().getId().equals(currentUser.getId())) {
			throw new AccessDeniedException("Solo l'autore può modificare questo post");
		}
	...
	public void delete(Long id, User currentUser) {
		...
		// AUTORIZZAZIONE: può eliminare il post l'AUTORE (sul proprio post) OPPURE un ADMIN (su qualsiasi post).
		// Un USER che prova a cancellare il post di un altro -> AccessDeniedException -> 403.
		boolean isAutore = esistente.getAutore().getId().equals(currentUser.getId());
		boolean isAdmin = currentUser.getRuolo() == Ruolo.ADMIN;
		if (!isAutore && !isAdmin) {
			throw new AccessDeniedException("Solo un ADMIN può eliminare il post di un altro utente");
		}
